...
public class RequestParamTest {
	...
	public String main(HttpServletRequest request) {	
		String year = request.getParameter("year");		// 필수입력이 아니기 때문에 에러가 나지 않는다.
...
//	public String main2(@RequestParam(name="year", required=false) String year) {   // 아래와 동일 
	public String main2(String year) {   				// 필수입력이 아니기 때문에 에러가 나지 않는다.	
...
//		public String main3(@RequestParam(name="year", required=true) String year) {   // 아래와 동일 
		public String main3(@RequestParam String year) {   // @RequestParam 이 붙어서 필수입력이기 때문에 입력하지 않으면 에러가 난다. (400)
...
//		http://localhost/ch2/requestParam4?year    ---->> year=""   
	...
	public String main5(@RequestParam(required=false, defaultValue="1") String year) { // 입력한 값은 없지만 컨트롤러에서는 1이라는 값이 들어간다. (null 일 떼, 1이 들어가기때문)
